...
def load_game_history():
    ...
    try:
        # 讀取P聯盟數據，明確指定列名
        games_df_p = pd.read_csv('cat/Model/dataset/p_currentgame.csv', names=[
        ...
        # 讀取T1數據，使用相同的列名
        t1_data = pd.read_csv('cat/Model/dataset/t_currentgame.csv', names=[
            'game_id', 'name', 'is_home', 'two_m', 'two', 'twop', 
            'trey_m', 'trey', 'treyp', 'ft_m', 'ft', 'ftp', 
            'points', 'reb', 'reb_o', 'reb_d', 'ast', 'stl', 
            'blk', 'turnover', 'pfoul'
...
clf = None  # 全域變數,存儲訓練好的模型

def train_model(team1, team2, is_home_team):
    global clf
    # 讀取數據
    games_df = load_game_history()
    
    # 讀取JSON數據
    p_player_data_23 = load_json('cat/Model/dataset/P_Players_Performance_23_24.json')
    t1_player_data_23 = load_json('cat/Model/dataset/T1_Players_performance_23_24.json')
    p_player_data_24 = load_json('cat/Model/dataset/P_Players_Performance_24_25.json')
    t1_player_data_24 = load_json('cat/Model/dataset/T1_Players_performance_24_25.json')

    p_team_data_23 = load_json('cat/Model/dataset/P_TeamStanding23_24.json')
    t1_team_data_23 = load_json('cat/Model/dataset/T1_TeamStanding23_24.json')
    p_team_data_24 = load_json('cat/Model/dataset/P_TeamStanding24_25.json')
    t1_team_data_24 = load_json('cat/Model/dataset/T1_TeamStanding24_25.json')

    t1_season_data = load_json('cat/Model/dataset/T1_Season_teams_performance_24_25.json')
    p_season_data = load_json('cat/Model/dataset/P_Season_teams_Performance_24_25.json')
    ...
    team1_recent = get_recent_performance(team1_name, games_df, 605)
    team2_recent = get_recent_performance(team2_name, games_df, 605)
    ...
    team1_recent_win_rate = team1_recent[3]
    team2_recent_win_rate = team2_recent[3]

    team1_point_diff = team1_recent[4]
    ...
    if team1_win_prob > team2_win_prob:
        team1_score_pred += score_adjustment * (team1_win_prob - 0.2)
        team2_score_pred -= score_adjustment * (team1_win_prob - 0.2)
    else:
        team1_score_pred -= score_adjustment * (team2_win_prob - 0.2)
        team2_score_pred += score_adjustment * (team2_win_prob - 0.2)
    ...
    stv = 5
    max_team1_score_pred = team1_score_pred + stv
    min_team1_score_pred = team1_score_pred - stv

    max_team2_score_pred = team2_score_pred + stv
    min_team2_score_pred = team2_score_pred - stv

    result = {
    "team1_win_prob": round(team1_win_prob, 2),
    "team2_win_prob": round(team2_win_prob, 2),
    "team1_score_range": {
        "min": round(min_team1_score_pred, 1),
        "max": round(max_team1_score_pred, 1)
    },
    "team2_score_range": {
        "min": round(min_team2_score_pred, 1),
        "max": round(max_team2_score_pred, 1)
    }}
