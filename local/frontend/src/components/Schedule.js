...
const Schedule = () => {
  ...
  useEffect(() => {
    // 獲取後端數據
    axios.get('http://localhost:8000/cat/schedules/')
      .then(response => {
      ...
      <div className='calendar-container'>
        <FullCalendar
          ...
          eventContent={(eventInfo) => (
            <div>
              <img src={getTeamIcon(eventInfo.event.extendedProps.team_home)} alt={eventInfo.event.extendedProps.team_home} style={{ width: '50px', height: '50px' }} />
              <img src={getTeamIcon(eventInfo.event.extendedProps.team_away)} alt={eventInfo.event.extendedProps.team_away} style={{ width: '50px', height: '50px', marginLeft: '10px' }} />
              <br />
